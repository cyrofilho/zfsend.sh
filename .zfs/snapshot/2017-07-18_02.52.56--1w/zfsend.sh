...
if [ ${ssh} != "yes" ]; then
		...
		if ! ${zfs} list ${dest_dataset_path} >/dev/null 2>&1; then
			...
			echo "${zfs} create -p ${dest_dataset_path}"
			if [ $? = 0 ]; then
				echo "done!"
			else
				exit
		...
		if ! ${zfs} list -t snapshot | grep ${dest_dataset}@ >/dev/null 2>&1; then
			...
			echo "${zfs} send -R ${latest_source_snap_fpath} | ${zfs} recv ${dest_dataset}"
			if [ $? = 0 ]; then
                                echo "done!"
				exit
                        else
		...
		if [ $latest_dest_snap = $latest_source_snap ]; then
			...
			echo "${zfs} send -I ${source_dataset_path}${latest_dest_snap} ${latest_source_snap_fpath} | ${zfs} recv ${dest_dataset}"
			if [ $? = 0 ]; then
                                echo "done!"
                        else
